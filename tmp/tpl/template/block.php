<?php 
					 ...
					 * @File /data/www/superweb/ad/tmp/tpl/template/block.php 
        ...
        <form class="form-horizontal form-row-seperated" action="" id="form_slide" class="form-horizontal" enctype="multipart/form-data" method="POST">
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption"> <i class="fa fa-shopping-cart"></i><?php echo lang('breadcrumb_block');?></div>
                    <div class="actions btn-set">
                        <button type="button" class="btn blue btn-info" data-toggle="modal" data-target="#copyBlock"><i class="fa fa-copy"></i> Sao chép block</button>
                    </div>
                </div>
    ...
    <div class="portlet rio_custom_a">
        ...
        <div class="portlet-title">
            <div class="caption">
                ...
                <span class="caption-helper">Danh sách block của :</span>&nbsp;
        ...
        <div class="portlet-title">
            ...
            <div class="actions">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-right">
                        <?php if(is_array($routerPosition)) { foreach($routerPosition as $k => $v) { ?>
                            <li>
                ...
                <div id="rio-drag">
                    <div class="rio-list-module">
                        <ul class="row" id="show_block_in_pos">
                            <?php if(isset($blockUse[$id])) { ?>
                            <?php if(is_array($blockUse[$id])) { foreach($blockUse[$id] as $k => $v) { ?>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-9">
                                                ...
                                                <?php if(is_array($mod_in_home)) { foreach($mod_in_home as $km => $vm) { ?>
                                                    <div class="list_modules">
                                                        <input class="modules_block_label" data="{<?=$v['id']?>" data-value="<?=$vm?>" <?php if(in_array($vm,$activeUse[$v['id']])) { ?> checked<?php } ?> type="checkbox" id="modules_block_<?=$v['id']?>_<?=$vm?>" name="modules_block_<?=$v['id']?>[]" value="<?=$vm?>">
                                        ...
                                        <div class="clear"></div>
                                            <div class="form-group">
                                             <label class="col-sm-3 control-label"><?php echo lang('short_textarea');?></label>
                                             <div class="col-sm-9">
                                                 <textarea class="form-control ckeditor span12" id="short_textarea<?=$v['id']?>" name="content" rows="6" style="width:420px"><?php if(isset($v['short'])) { ?><?=$v['short']?><?php } ?></textarea>
                                             </div>
                                        </div>
                                    </div>
...
<!-- Modal -->
<div class="modal fade" id="copyBlock" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
<form role="form" id="formCopy" action="/template-Language-copyBlocks-lang-<?=$get_lang?>">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Sao chép blocks</h4>
      </div>
      <div class="modal-body">
        <div class="note note-warning">
            <p class="block">
                - Bạn có thể sao chép được toàn bộ blocks sang ngôn ngữ được định sẵn.<br/>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <select name="copy[copyBlocks]" class="form-control">
                    <?php if(is_array($url_lang)) { foreach($url_lang as $k_lang => $v_lang) { ?>
            ...
            <div class="col-md-6">
                <label>Bạn có muốn xóa tất cả block ?</label> <br/>
                <input class="form-control" value="1" type="checkbox" name="copy[emptyBlocks]"> Làm rỗng trước khi sao chép
            </div>
...
$(document).ready(function() {
    ...
    $(function() {
        ...
        $('body').on('click', '.reset_name', function(event) {
            var tmp = $(this).parents('.prive_posive');
            var textDf=tmp.find('input[name="text_default_hd"]').val();
            tmp.find('input[type="text"]').val(textDf);
        });
        ...
        function getArray(act, id, idpos) {
            ...
            if (act == 'use-block') {
                ...
                update_status_block(arrJs, id, idpos, "hide");
            } else if (act == 'show-block') {
                var sArr = {
                    status: 1
                };
                var arrJs = JSON.stringify(sArr);
                update_status_block(arrJs, id, idpos, "show");
        ...
        function update_status_block(arrJs, idblock, idposition, status) {
        ...
        function checkBlock() {
            var parrentMove = $("#blockMoveItem").text();

            // console.log(parrentMove);
        ...
        $("#store_content").on('click', '.rlm-use', function() {
            ...
            var newBlock = "<li  data-idblock=" + id_block + " data-file=" + file_block + " data-module=" + module_name_block + " data-module-id=" + module_id_block + " data-position=" + <?=$id?> + " class='bounceIn' >" + currentClickDiv.html() + "</li>";
            divDrag.find("ul").prepend(newBlock);
            getArray('use-block');

            currentClickDiv.find(".rlm-item").css({
                "background": "#C2C2C2",
                "opacity": "0.5"
            });
            ...
            $(".divMove").css({
                top: currentClickDiv.position().top,
                left: currentClickDiv.position().left
            });
            $(".divMove").animate({
                top: 10,
            ...
            //   // top: divDragOffset.top - $(window).scrollTop(),
            //    // left: divDragOffset.left - $(window).scrollTop(),
            //     //opacity: 0,
            //     //visibility:"hidden"
            // }, 1000, function() {
        ...
        $("#rio-drag").on('click', '.rlm-del', function() {
            var _this = $(this);
            bootbox.dialog({
                message: '<li class="list-group-item list-group-item-warning">Cảnh báo: Bạn đồng ý với xóa block đồng nghĩa với việc toàn bộ dữ liệu về block này sẽ trở về mặc định của hệ thống. Bạn có chắc chắn xóa ?</li>',
                title: "Xoá block",
                buttons: {
                    success: {
                        label: "Đồng ý",
                        className: "green",
                        callback: function() {
                            var parentDiv = _this.parent().parent().parent().parent();
                            var id = parentDiv.attr('data-idblock');
