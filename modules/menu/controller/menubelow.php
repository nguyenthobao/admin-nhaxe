...
if(empty($get_lang)){
	...
	if (!empty($id)) {
		foreach ($lang_use as $key => $v) {
			$url_lang[$v]['url'] = $_B['home'].'/'.$mod.'-menubelow-'.$id.'-'.$v;
		}
	}else{
		//Get ngôn ngữ đã config trong db để set làm menu lang
		foreach ($lang_use as $k => $v) {
			if ($v=='vi') {
				$url_lang[$v]['url'] = $_B['home'].'/'.$mod.'-menubelow-lang-vi';	
			}else{
...
$menubelow = new MenuBelow();
	...
	$info_data      =  $menubelow->getMenuBelow($id);

	$content_module = $_B['temp']->load('menubelow');
