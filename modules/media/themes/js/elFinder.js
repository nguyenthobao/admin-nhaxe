...
window.elFinder = function(node, opts) {
	...
	var self = this,
		 ...
		 **/
		cwdOptions = {
			path          : 'http://dev2.webbnc.vn/cdn/',
			url           : 'http://dev2.webbnc.vn/cdn/',
			tmbUrl        : 'http://dev2.webbnc.vn/cdn/',
			disabled      : [],
			separator     : '/',
			archives      : [],
			extract       : [],
	...
	this.tmb = function(hash) {
		var file = files[hash],
			...
			//url = 'http://upload.webbnc.vn/media/';
...
elFinder.prototype = {
	...
	startDir : function() {
		...
		if (locHash && locHash.match(/^#nbh_/)) {
			return locHash.replace(/^#nbh_/, '');
		} else {
			return this.lastDir();
		}
	},
