...
class BncUpload {
	...
	public function __construct($options = null) {
		...
		$this->options = array(
			'sv_url'            => 'http://upload2.webbnc.vn/index.php?data=',
			'sv_url_del'        => 'http://upload2.webbnc.vn/nbh.php',
			'sv_url_write_file' => 'http://upload2.webbnc.vn/nbhWriteFile.php',
			'thumb'             => array('100'),
	...
	public function upload($idw = 1, $module = null, $name_field = null, $name_up = '', $thumb = null) {
		...
		if ($this->options['type_file'] == 'audio' || $this->options['type_file'] == 'flash' || $this->options['type_file'] == 'document') {
			...
			if (!empty($name_up)) {
				$name_file = $timestamp . '_' . $this->fix_file_name($name_up) . '.' . $ext;
			}
			$thumb = ($thumb != null) ? $thumb : $this->options['thumb'];

			$move=move_uploaded_file($file_, $this->options['tmp_upload'] . '/' . $name_file);
			...
			// 		include_once DIR_HELPER . 'upload.helper/watermark.php';
			// 		@$file_setting = file_get_contents('http://upload2.webbnc.vn/nbhWaterMark/' . $idw . '/setting.txt');
					
			// 		if (!empty($file_setting)) {
			// 			$wt        = json_decode($file_setting, 1);
			// 			$watermark = new nhbWatermark($wt['image']);

			// 			if ($wt['type'] == 1) {
			// 				//Kiểu text
			// 				$watermark->load($this->options['tmp_upload'] . '/' . $name_file);
			// 				$font = DIR_HELPER . 'upload.helper/fonts_watermark/' . $wt['font'];
			// 				$watermark->text($wt['text'], $font, $wt['size'], $wt['color'], $wt['x'], $wt['y'], $wt['rotate']);
			// 				$imag=$watermark->save($this->options['tmp_upload'] . '/' . $name_file);
			// 				if(isset($_COOKIE['truong'])){
			// 					echo '<pre>';
			// 					print_r($imag);
			// 					echo '</pre>';
