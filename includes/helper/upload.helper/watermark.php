...
class nhbWatermark {
	 ...
	 *
	 * @param string		$overlay		An image filename or a nhbWatermark object
	 ...
	 *
	 */
	function overlay($overlay, $position = 'null', $opacity = 1, $x_offset = 0, $y_offset = 0, $rotate = 0) {
		
		// Load overlay image
		if( !($overlay instanceof nhbWatermark) ) {
			$overlay = new nhbWatermark($overlay);
		}
		...
		switch (strtolower($position)) {
			...
			// 	$y = ($this->height / 2) - ($overlay->height / 2) + $y_offset;
			default:
				$y = 0 + ($y_offset*$this->height)/100;
				$x = 0 + ($x_offset*$this->width)/100;
				$ov = $this->rotate($overlay->image,$rotate);
				$overlay->image = $ov['image'];
				$overlay->width = $ov['width'];
				$overlay->height = $ov['height'];
	...
	/**
	 * Rotate an image
	 * HuongNB<nguyenbahuong156@gmail.com>
	 **/
	function rotate($ov,$angle, $bg_color = 0) {
		
		$new = imagerotate($ov, -($this->keep_within($angle, -360, 360)),imageColorAllocateAlpha($ov, 0, 0, 0, 127));
		imagesavealpha($new, true);
		imagealphablending($new, false);
		// Cập nhật lại ảnh watermark
		$overlay['width']  = imagesx($new);
		$overlay['height'] = imagesy($new);
		$overlay['image']  = $new;
	...
	/**
	 * NBH<nguyenbahuong156@gmail.com>
	 */
	 ...
	 *
	 */
	function text($text, $font_file, $font_size = 12, $color = '#000000', $x_offset = 0, $y_offset = 0, $angle =0) { 
		...
		$x = ($x_offset*$this->width)/100;
		$y = ($y_offset*$this->height)/100;
