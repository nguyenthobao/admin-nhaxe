...
class MysqliDb {
	...
	public function num_rows($tableName) {
		$this->_query = "SELECT * FROM " . self::$prefix . $tableName;
	...
	/**
	 * Check exist table 
	 * Author Quang Chau Tran
	 * Modify Nguyen Xuan Truong
	 * @return boolean
	 */
	public function table_exist($tableName) {
		
		
		$this->_query = "SHOW TABLES LIKE '" . self::$prefix . $tableName . "'";
		$stmt         = $this->_buildQuery();
		...
		foreach ($tableData as $column => $value) {
			...
			switch ($key) {
				...
				case '[N]':
					...
					break;
				default:
					die('Wrong operation'.$this->_query);
			}
		}
